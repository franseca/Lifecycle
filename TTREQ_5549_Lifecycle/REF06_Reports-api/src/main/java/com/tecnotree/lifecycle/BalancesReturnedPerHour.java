...
public class BalancesReturnedPerHour {
    
	//CONSTANTES
	private static String PROP_FILE_NAME = "configurationBalancesReturnedPerHour.properties";
  			...
  			if(process.equalsIgnoreCase("A")) {//SI EL PROCESO ES AUTOMATICO
  				...
  				try {
			  		...
			  		loadProperties(PROP_FILE_NAME);
	  		  		...
	  		  		System.out.println(dateFormat.format(new Date()) + " - Finished Balances Returned per Hour Report.");
	  		  		return;
	  		  	}
  				...
  				try {
			  		...
			  		logger = new Tn3Logger("BalancesReturnedPerHour", log_outputDirectory);
			  		
			  		logger.info("Command executed: BalancesReturnedPerHour "+ args[0]);
			  		...
			  		logger.info("Starting Balances Returned per Hour Report...");
			  		
					//GENERO EL ARCHIVO DE SALIDA
					generateBalancesReturnedPerHour();
		  		...
		  		try {	
			  		...
			  		logger.info("Command executed: BalancesReturnedPerHour "+ args[0] + " " + args[1] + " " + args[2]);
			  		System.out.println(dateFormat.format(new Date()) + " - Command executed: BalancesReturnedPerHour "+ args[0] + " " + args[1] + " " + args[2]);
			  						  		
			  		logger.info("Properties file load.");
			  		System.out.println(dateFormat.format(new Date()) + " - Starting Balances Returned per Hour Report...");
			  		logger.info("Starting Balances Returned per Hour Report..");
	...
	/**
	 * Method generates a file with the balances returned per hour
	 * 
	 * @throws ClassNotFoundException
	 * @throws SQLException
	 * @throws IOException 
	 */
	private static void generateBalancesReturnedPerHour() throws ClassNotFoundException, SQLException, IOException {
		...
		if(process.equals("A")) {
			...
			query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d %H\"),'h','|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = 1\r\n"
						+ "AND CDV_RECORDS.STATUS = 'DEVUELTO'\r\n"
			...
			if(dateOption == 1) {//SI SE INGRESO EL PARAMTERO FECHA EN EL FORMATO yyyy-MM-dd
				...
				query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d\"),'|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = " + lastRecords + "\r\n"
				...
				query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m\"),'|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = " + lastRecords + "\r\n"
		...
		System.out.println(dateFormat.format(new Date()) + " - Finished Balances Returned per Hour Report.");
		logger.info("Finished Balances Returned per Hour Report.");
	}
