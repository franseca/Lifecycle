...
public class TransactionsPerNumber {
    
	//CONSTANTES
	private static String PROP_FILE_NAME = "configurationTransactionsPerNumber.properties";
  		...
  		if(args.length == 0) {//SI NO SE PASARON PARAMETROS
  			...
  			if(process.equalsIgnoreCase("A")) {//SI EL PROCESO ES AUTOMATICO
  				...
  				try {
			  		...
			  		logger = new Tn3Logger("TransactionsPerNumber", log_outputDirectory);
			  		
			  		logger.info("Command executed: TransactionsPerNumber "+ args[0]);
			  		System.out.println(dateFormat.format(new Date()) + " - Command executed: TransactionsPerNumber "+ args[0]);
			  						  		
			  		logger.info("Properties file load.");
			  		System.out.println(dateFormat.format(new Date()) + " - Starting Transactions per Number Report...");
			  		logger.info("Starting Transactions per Number Report...");
			  		
					//GENERO EL ARCHIVO DE SALIDA
			  		generateTransactionsPerNumber();
  				...
  				if(args.length < 2) {//SI NO SE PASARON PARAMETROS
		  		...
		  		try {	
			  		...
			  		logger.info("Command executed: TransactionsPerNumber "+ args[0] + " " + args[1]);
			  		System.out.println(dateFormat.format(new Date()) + " - Command executed: TransactionsPerNumber "+ args[0] + " " + args[1]);
	...
	/**
	 * Method generates a file with the transactions per number
	 * 
	 * @throws ClassNotFoundException
	 * @throws SQLException
	 * @throws IOException 
	 */
	private static void generateTransactionsPerNumber() throws ClassNotFoundException, SQLException, IOException {
		...
		if(process.equals("A")) {
			...
			query = "SELECT CONCAT(A.CREATION_DATE,'|',A.MSISDN,'|',A.STATUS,'|',A.BALANCE,'|',A.POST_BALANCE) RECORD\r\n"
					+ "FROM\r\n"
					+ "(SELECT DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d %H:%i:%s\") CREATION_DATE,CDV_RECORDS.MSISDN,\r\n"
					+ "case \r\n"
					+ "  when CDV_RECORDS.STATUS = 'CONGELADO' then 'CDV_Congelar_Saldo'\r\n"
					+ "  when CDV_RECORDS.STATUS = 'DEVUELTO' then 'CDV_Devolver_Saldo'\r\n"
					+ "  when CDV_RECORDS.STATUS = 'EXPIRADO_CS' then 'CDV_Expirar_Saldo'\r\n"
					+ "  else 'NO DEFINED'\r\n"
					+ "end as STATUS,\r\n"
					+ "case \r\n"
					+ "  when CDV_RECORDS.STATUS = 'CONGELADO' then IFNULL(if(LOCATE('0.',convert(FORMAT(CDV_RECORDS.BALANCE/1000,3),NCHAR))=1,REPLACE(convert(FORMAT(CDV_RECORDS.BALANCE/1000,3),NCHAR),'0.','.'),FORMAT(CDV_RECORDS.BALANCE/1000,3)),0)\r\n"
					+ "  when CDV_RECORDS.STATUS = 'DEVUELTO' then IFNULL(if(LOCATE('0.',convert(FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3),NCHAR))=2,REPLACE(convert(FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3),NCHAR),'0.','.'),FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3)),0)\r\n"
					+ "  when CDV_RECORDS.STATUS = 'EXPIRADO_CS' then IFNULL(if(LOCATE('0.',convert(FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3),NCHAR))=2,REPLACE(convert(FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3),NCHAR),'0.','.'),FORMAT(CDV_RECORDS.BALANCE/1000 * -1,3)),0)\r\n"
					+ "  else '0'\r\n"
					+ "end as BALANCE,\r\n"
					+ "case \r\n"
					+ "  when CDV_RECORDS.STATUS = 'CONGELADO' then IFNULL(if(LOCATE('0.',convert(FORMAT(CDV_RECORDS.BALANCE/1000,3),NCHAR))=1,REPLACE(convert(FORMAT(CDV_RECORDS.BALANCE/1000,3),NCHAR),'0.','.'),FORMAT(CDV_RECORDS.BALANCE/1000,3)),0)\r\n"
					+ "  when CDV_RECORDS.STATUS = 'DEVUELTO' then 0\r\n"
					+ "  when CDV_RECORDS.STATUS = 'EXPIRADO_CS' then 0\r\n"
					+ "  else '0'\r\n"
					+ "end as POST_BALANCE\r\n"
					+ "FROM CDV_RECORDS\r\n"
		...
		System.out.println(dateFormat.format(new Date()) + " - Finished Transactions per Number Report.");
		logger.info("Finished Transactions per Number Report.");
	}
	...
	private static void sendFtp(String ftp_ip, String ftp_port, String ftp_user, String ftp_pass, boolean ftp_sftp, String ftp_path, String file_name, String absolutePathFile) throws JsonProcessingException {
		...
		logJson.setReport("TransactionsPerNumber");
