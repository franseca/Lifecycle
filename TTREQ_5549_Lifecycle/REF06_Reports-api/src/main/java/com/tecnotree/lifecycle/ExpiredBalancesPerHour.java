...
public class ExpiredBalancesPerHour {
    
	//CONSTANTES
	private static String PROP_FILE_NAME = "configurationExpiredBalancesPerHour.properties";
  			...
  			if(process.equalsIgnoreCase("A")) {//SI EL PROCESO ES AUTOMATICO
  				...
  				try {
			  		...
			  		loadProperties(PROP_FILE_NAME);
	  		  		...
	  		  		System.out.println(dateFormat.format(new Date()) + " - Finished Expired Balances per Hour Report.");
	  		  		return;
	  		  	}
  				...
  				try {
			  		...
			  		logger = new Tn3Logger("ExpiredBalancesPerHour", log_outputDirectory);
			  		
			  		logger.info("Command executed: ExpiredBalancesPerHour "+ args[0]);
			  		System.out.println(dateFormat.format(new Date()) + " - Command executed: ExpiredBalancesPerHour "+ args[0]);
			  						  		
			  		logger.info("Properties file load.");
			  		System.out.println(dateFormat.format(new Date()) + " - Starting Expire Balances per Hour Report...");
			  		logger.info("Starting Expire Balances per Hour Report...");
			  		
					//GENERO EL ARCHIVO DE SALIDA
					generateExpiredBalancesPerHour();
		  		...
		  		try {	
			  		...
			  		logger.info("Command executed: ExpiredBalancesPerHour "+ args[0] + " " + args[1] + " " + args[2]);
			  		System.out.println(dateFormat.format(new Date()) + " - Command executed: ExpiredBalancesPerHour "+ args[0] + " " + args[1] + " " + args[2]);
	...
	/**
	 * Method generates a file with the expired balances per hour
	 * 
	 * @throws ClassNotFoundException
	 * @throws SQLException
	 * @throws IOException 
	 */
	private static void generateExpiredBalancesPerHour() throws ClassNotFoundException, SQLException, IOException {
		...
		if(process.equals("A")) {
			...
			query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d %H\"),'h','|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE_T,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = 1\r\n"
						+ "AND DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d\") = '" + date + "'\r\n"
						+ "AND CDV_RECORDS.STATUS = 'EXPIRADO_CS'\r\n"
						+ "GROUP BY DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d %H\")\r\n"
						+ "ORDER BY DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d %H\") DESC;";
		}else {
			
			if(dateOption == 1) {//SI SE INGRESO EL PARAMETRO FECHA EN EL FORMATO yyyy-MM-dd
				...
				query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m-%d\"),'|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE_T,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = " + lastRecords + "\r\n"
				...
				query = "SELECT CONCAT(DATE_FORMAT(CDV_RECORDS.CREATION_DATE,\"%Y-%m\"),'|',COUNT(*),'|',FORMAT(SUM(IFNULL(CDV_RECORDS.BALANCE_T,0))/1000 * -1,3)) RECORD\r\n"
						+ "FROM CDV_RECORDS\r\n"
						+ "WHERE CDV_RECORDS.LAST = " + lastRecords + "\r\n"
		...
		System.out.println(dateFormat.format(new Date()) + " - Finished Expired Balances per Hour Report.");
		logger.info("Finished Expired Balances per Hour Report");
	}
	...
	private static void sendFtp(String ftp_ip, String ftp_port, String ftp_user, String ftp_pass, boolean ftp_sftp, String ftp_path, String file_name, String absolutePathFile) throws JsonProcessingException {
		...
		logJson.setReport("ExpiredBalancesPerHour");
